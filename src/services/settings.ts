...
export default class Settings extends EventDispatcher<SettingsEventDefinitions> {
  ...
  public static new = (drive: Drive): Promise<Settings> =>
    new Promise((resolve, reject) => {
      drive
        .searchFiles([], ConfigFile.name)
        .then(({ files }) => {
          if (!files) {
            ...
            drive
              ...
              .then((config: ImgstorConfig) => {
                ...
                const settings = new Settings(
                  merge({ ...Settings.DefaultConfig }, config),
                  drive,
                );
                resolve(settings);
              })
  ...
  public async saveConfig(config: ImgstorConfig): Promise<ImgstorConfig> {
    ...
    const updatedConfigBlob = new Blob([updatedConfigString], {
      type: ConfigFile.type,
    });
    const file = new File([updatedConfigBlob], ConfigFile.name, {
      type: ConfigFile.type,
    });
