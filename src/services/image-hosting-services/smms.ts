...
export default class Smms {
  ...
  private static readonly UPLOAD_SIZE_LIMIT_BYTES = 5 * 1024 ** 2; //5MB
  ...
  public async Upload(
    ...
    if (file.size > Smms.UPLOAD_SIZE_LIMIT_BYTES) {
      throw new Error(
        `Image file size exceeds the limit of ${Smms.UPLOAD_SIZE_LIMIT_BYTES / 1024 ** 2}MB.`,
      );
    }
    ...
    if (shouldSave && separatePreviewUpload) {
      if (!imageFile.preview) {
        throw new Error(
          "Preview image file is required when separate preview upload is enabled.",
        );
      }
      ...
      if (previewFile.size > Smms.UPLOAD_SIZE_LIMIT_BYTES) {
        throw new Error(
          `Preview image file size exceeds the limit of ${Smms.UPLOAD_SIZE_LIMIT_BYTES / 1024 ** 2}MB.`,
        );
      }
    }
    ...
    const previewRequest = (() => {
      if (shouldSave && separatePreviewUpload && imageFile.preview) {
        return Smms.ConvertPngToWebpThumbnail(imageFile.preview.file).then(
          (thumbnail) => {
            const form = new FormData();
    ...
    try {
      const mainResult: SmmsUploadResult = await mainRequest.then((res) =>
        res.json(),
      );

      const previewResult = previewRequest
        ? await previewRequest.then((res) => res.json())
        : undefined;
  ...
  private static async ConvertPngToWebpThumbnail(pngFile: File): Promise<File> {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            const webpThumbnail = new File([blob], THUMBNAIL_FILE_NAME, {
              type: WEBP_MIME_TYPE,
            });
