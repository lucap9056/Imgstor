...
function Insert(db: Database, image: ImgstorImage): void {
  ...
  const stmt = db.prepare(
    `INSERT INTO ${TABLE_NAME}(${columnsToInsert}) VALUES(${columns.map((_) => "?").join(",")})`,
  );
  stmt.run([
    name,
    mimeType,
...
function Update(db: Database, image: ImgstorImage): void {
  ...
  const stmt = db.prepare(
    `UPDATE ${TABLE_NAME} SET ${columnsToUpdate} WHERE ${COLUMNS.imageId}=?`,
  );
...
function Delete(db: Database, id: string): void {
  const stmt = db.prepare(
    `DELETE FROM ${TABLE_NAME} WHERE ${COLUMNS.imageId}=?`,
  );
  stmt.run([id]);
  stmt.free();
}
