...
function Get(
  ...
  const columns =
    include.length > 0 ? include.map((i) => `t.${i}`).join(",") : "t.*";

  const query = `SELECT ${columns} FROM ${TABLE_NAME} it JOIN ${Tag.TABLE_NAME} t ON it.${COLUMNS.tag} = t.${Tag.COLUMNS.tagId} WHERE ${COLUMNS.image}=?`;
...
function Insert(db: Database, image: string, tag: string): void {
  const stmt = db.prepare(
    `INSERT INTO ${TABLE_NAME} (${COLUMNS.image},${COLUMNS.tag}) VALUES (?,?)`,
  );
  stmt.run([image, tag]);
  stmt.free();
}
...
function Delete(
  ...
  if (tagID === undefined) {
    ...
    const stmt = db.prepare(
      `DELETE FROM ${TABLE_NAME} WHERE ${COLUMNS.image}=? AND ${COLUMNS.tag}=?`,
    );
