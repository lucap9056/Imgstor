...
const ImageComponenet: React.FC<Props> = ({ image, onload }) => {
  ...
  const [title] = useState(
    hostingService
      ...
      : t("image.hosting-service.not-found", {
          hostingService: image.hostingServiceId,
        }),
  );
  ...
  const InvalidImageUrl = (): string => {
    ...
    if (
      ...
      const scalingFactor =
        actualHeight * defaultAspectRatio < actualWidth
  ...
  return (
    <div className={styles.image} ref={ref}>
      ...
      {title && (
        <div className={styles.image_title} onClick={HandleFocusView}>
          {title}
        </div>
      )}
    </div>
