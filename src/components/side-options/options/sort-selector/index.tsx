...
const SortSelector: React.FC<Props> = ({ defaultValue, onchange, hide }) => {
  ...
  const sortOptionsInvisible = useCallback(
    (e: MouseEvent) => {
      if (
        sortOptionsVisible &&
        sortSelectorRef.current &&
        !sortSelectorRef.current.contains(e.target as Node)
      ) {
  ...
  return (
    <div ref={sortSelectorRef} className={styles.images_sort_select}>
      <span
        className={styles.images_sort_selected_option}
        onClick={() => setSortOptionsVisible(true)}
      >
        {t(`image.sort.${sort}`)}
      </span>
      {sortOptionsVisible && (
        <div className={styles.images_sort_options}>
          <li
            className={styles.images_sort_option}
            onClick={handleSelectSort}
            data-value={ImgstorImage.SORT.NEWEST}
          >
            {t("image.sort.newest")}
          </li>
          <li
            className={styles.images_sort_option}
            onClick={handleSelectSort}
            data-value={ImgstorImage.SORT.OLDEST}
          >
            {t("image.sort.oldest")}
          </li>
          <li
            className={styles.images_sort_option}
            onClick={handleSelectSort}
            data-value={ImgstorImage.SORT.DEFAULT}
          >
            {t("image.sort.default")}
          </li>
        </div>
      )}
    </div>
