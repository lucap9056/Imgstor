...
const HostingServicesSelect: React.FC<Props> = ({ imgstor, onchange }) => {
  ...
  const [selectedHostingService, SetSelectedHostingService] =
  ...
  return (
    <div className={styles.image_hosting_services}>
      {Object.values(hostingServices).map(({ hostingServiceId, NAME }) => (
        <div
          key={hostingServiceId}
          className={styles.image_hosting_service}
          data-selected={
            hostingServiceId === selectedHostingService.hostingServiceId
          }
