...
import type { ImageData } from "services/image-hosting-services/import-external";
...
const ImportExternalComponent: React.FC<Props> = ({ onchange }) => {
  ...
  const HandleBlur = () => {
    ...
    if (imageUrl !== "") {
      ...
      const file = new File([new Blob([imageDataStr])], "", {
        type: "text/plain",
      });
  ...
  return (
    <div className={styles.uploader_import}>
      ...
      <input
        ...
        onBlur={HandleBlur}
        placeholder={
          imageUrl ? imageUrl : t("uploader.import.label.preview-url")
        }
      />
    </div>
  );
