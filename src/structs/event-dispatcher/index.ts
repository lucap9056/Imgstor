...
class EventDispatcher<EventDefinitions extends Record<string, any>> {
   ...
   */
  public on<EventName extends keyof EventDefinitions>(
    event: EventName,
    handler: (payload: EventDefinitions[EventName]) => void,
  ) {
   ...
   */
  public off<EventName extends keyof EventDefinitions>(
    event: EventName,
    handler: (payload: EventDefinitions[EventName]) => void,
  ) {
   ...
   */
  protected emit<EventName extends keyof EventDefinitions>(
    event: EventName,
    value: EventDefinitions[EventName],
  ) {
    const handlers = this.events.get(event);
    if (!handlers) return;
   ...
   */
  public once<EventName extends keyof EventDefinitions>(
    event: EventName,
    handler: (payload: EventDefinitions[EventName]) => void,
  ) {
