...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      port: env.VITE_SERVER_PORT ? parseInt(env.VITE_SERVER_PORT, 10) : 443,
      mimeType: {
        "application/wasm": ["wasm"],
      },
      https: {
        cert: env.VITE_SSL_CRT
          ? fs.readFileSync(path.resolve(__dirname, env.VITE_SSL_CRT))
          : undefined,
        key: env.VITE_SSL_KEY
          ? fs.readFileSync(path.resolve(__dirname, env.VITE_SSL_KEY))
          : undefined,
      },
    },
  };
